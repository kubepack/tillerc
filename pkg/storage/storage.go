...
package storage // import "github.com/appscode/tillerc/pkg/storage"
...
func (s *Storage) Get(name string, version int32) (*hapi.Release, error) {
...
func (s *Storage) Create(rls *hapi.Release) error {
	log.Printf("Create release %q (v%d) in storage\n", rls.Name, rls.Spec.Version)
	return s.Driver.Create(makeKey(rls.Name, rls.Spec.Version), rls)
}
...
// does not exist.
func (s *Storage) Update(rls *hapi.Release) error {
	log.Printf("Updating %q (v%d) in storage\n", rls.Name, rls.Spec.Version)
	return s.Driver.Update(makeKey(rls.Name, rls.Spec.Version), rls)
}
...
// does not exist.
func (s *Storage) Delete(name string, version int32) (*hapi.Release, error) {
